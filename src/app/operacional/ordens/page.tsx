...
export default function OrdensServicoPage() {
  ...
  useEffect(() => {
    if (!isAuthenticating && !user) {
      router.push('/login?redirect=/operacional/ordens');
    }
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <Button onClick={() => router.push('/operacional/atendimentos/novo')}>
                ...
                {filteredOrdens.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell className="text-right">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 mr-1" onClick={() => router.push(`/operacional/producao?agendamentoId=${order.id}`)} title="Ir para Produção" disabled={isSubmitting}>
